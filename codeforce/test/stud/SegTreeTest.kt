...
class SegTreeTest {
    @Test
    fun testZero() {
        var seg = SegTree<String>(0, SegTreeTest::op, SegTreeTest::e)
        assertEquals("$", seg.allProd())

        seg = SegTree<String>(SegTreeTest::op, SegTreeTest::e)
        assertEquals("$", seg.allProd())
    }

    @Test
    fun testOne() {
        var s = SegTree<String>(1, SegTreeTest::op, SegTreeTest::e);
        assertEquals("$", s.allProd());
        assertEquals("$", s.get(0));
        assertEquals("$", s.prod(0, 1));
        s.set(0, "dummy");
        assertEquals("dummy", s.get(0));
        assertEquals("$", s.prod(0, 0));
        assertEquals("dummy", s.prod(0, 1));
        assertEquals("$", s.prod(1, 1));
    }

    @Test
    fun testCompareNaive() {
        for (n in 0 until 30) {
            var seg0 = SegTreeNaive(n, SegTreeTest::op, SegTreeTest::e)
            var seg1 = SegTree(n, SegTreeTest::op, SegTreeTest::e);
            for (i in 0 until n) {
                var s = ""
                s += 'a' + i
                seg0.set(i, s);
            ...
            for (l in 0..n) {
                for (r in l..n) {
                    y = seg1.prod(l, r);
                    var leq_y = fun(x: String): Boolean { return x.length <= y.length }
                    assertEquals(seg0.maxRight(l, leq_y), seg1.maxRight(l, leq_y));
                    assertEquals(
                        seg0.maxRight(l, leq_y),
                        seg1.maxRight(l) { x: String -> x.length <= y.length })
                }
            }
            ...
            for (r in 0..n) {
                for (l in 0..r) {
                    y = seg1.prod(l, r);
                    var leq_y = fun(x: String): Boolean { return x.length <= y.length }
                    assertEquals(seg0.minLeft(r, leq_y), seg1.minLeft(r, leq_y));

                    assertEquals(
                        seg0.minLeft(r, leq_y),
                        seg1.minLeft(r) { x: String -> x.length <= y.length })
                }
            }
        }
    }
    ...
    class SegTreeNaive<S>(val n: Int, val op: (S, S) -> S, val e: () -> S) {
        val d: MutableList<S> = MutableList<S>(n) { e() }
